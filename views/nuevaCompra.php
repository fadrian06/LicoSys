...
if (Session::get('cargo') === 'a') :
  ...
  if (Session::has('proveedorID')) {
    $proveedor = getRegistro('SELECT * FROM proveedores WHERE id=' . Session::get('proveedorID'));
  }
  ...
  $mostrarLista = Session::has('proveedorID')
    ? ''
    : 'w3-hide';
  ...
  $tooltipRegistrarProducto = BareUI::render('components/tooltip', [
    'slot' => 'Registrar Producto',
  ]);
  ...
  $inputNombre = generarInput(NombreInput::NOMBRE_NEGOCIO, $label, 'Nombre del proveedor');
