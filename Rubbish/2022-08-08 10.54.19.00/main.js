...
if (document.querySelectorAll(".botonNegocio")) {
	...
	$botonesNegocios.forEach(function (boton) {
		const idNegocio = boton.id[boton.id.length - 1];
		...
		$botonesActualizar.forEach(function (botonActualizar) {
			botonActualizar.addEventListener("click", function () {
				botonActualizar.previousElementSibling.classList.replace("w3-show", "w3-hide");
				botonActualizar.nextElementSibling.classList.replace("w3-hide", "w3-show");
				botonActualizar.classList.replace("w3-show", "w3-hide");
			})
		})
	})
}

formularioModal("#botonAgregarNegocio", "#formularioRegistrarNegocio", ".overlayModal");