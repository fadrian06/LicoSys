<?php
	...
	if (isset($_POST["registrarProducto"])):
		...
		$idNegocio = (int) $_SESSION["negocio"];
		...
		if(mysqli_num_rows($resultado)>0):
			...
			$sql="INSERT INTO inventario VALUES('$codigo', '$nombre', $stock, '$excento', $precio, $idNegocio, $cedulaUsuario)";
	...
	if (isset($_POST["registrarProveedor"])):
		$nombre = ESCAPAR_STRING($_POST["nombreProveedor"]);
		$cedulaUsuario = (int) $_SESSION["idUsuario"];
		$idNegocio = (int) $_SESSION["negocio"];
		...
		if(mysqli_num_rows($resultado)>0):
			$notificacion = "
				<script>
					...
					document.querySelector('#registrarProveedor').click();
				</script>
			";
		else:
			$sql="INSERT INTO proveedor VALUES(NULL, '$nombre', $cedulaUsuario, $idNegocio)";
			$resultado=mysqli_query($conexion, $sql);
			if(mysqli_affected_rows($conexion)>0):
				$notificacion = "
					<script>
...
<html lang="es">
	...
	<body class="w3-light-grey">
			...
			<form action="" method="POST" class="w3-margin-top w3-row formularioModal w3-padding-24 w3-display-container w3-center w3-white w3-card w3-round-large animate__animated animate__fadeInUp animate__faster w3-hide" id="formularioRegistrarProveedor">
				...
				<section class="w3-padding-16 w3-container w3-margin-top w3-margin-left w3-margin-right">
					<div class="input">
						...
						<input type="text" name="nombreProveedor" placeholder="Nombre del Proveedor" required>
					</div>
				</section>
				<div class="submit w3-margin-top w3-container">
		...
		<?php elseif ($usuariosActivo && $_SESSION["cargo"] == "a"): ?>
			...
			<form action="" method="POST" class="w3-margin-top w3-row formularioModal w3-padding-24 w3-display-container w3-center w3-white w3-card w3-round-large animate__animated animate__fadeInUp animate__faster w3-hide" id="formularioRegistrarUsuario">
				<span class="w3-button w3-hover-red w3-xlarge w3-display-topright">&times;</span>
