...
if ($path === '/dashboard.php' && key_exists('userID', $_SESSION)):
  ...
  /*----------  Inventario agotado  ----------*/
  $productos = $manager::table('inventario')
    ->get(['id', 'producto', 'stock'])
    ->toArray();
  ...
  foreach ($productos as $index => $producto):
    ...
    if (!$producto['stock'])
      Scripts::pushInline(<<<JS
        ...
        let intervalo{$index} = setInterval(() => {
          alerta('{$producto['producto']} está AGOTADO').show()
        }, $tiempo)

        setTimeout(
          () => clearInterval(intervalo{$index}),
          $tiempo * 10 /* 10 minutos */
        )
      JS);
...
try {
  $negocios = $manager::table('negocios')
    ->where('activo', true)
    ->get()
    ->toArray();
...
$baseHref = str_replace(['index.php', 'dashboard.php'], '', $_SERVER['SCRIPT_NAME']);
