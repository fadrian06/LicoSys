...
const moduloClientes = (contenedor: HTMLElement) => {
  ...
  mostrarDetails(contenedor.querySelector("details"));
  registrarCliente(
    contenedor.querySelector("#registrarCliente"),
    "views/clientes.php",
  );
};
...
const moduloFinanzas = (contenedor) => {
  ...
  $('input[type="radio"]').on("click", (e) => {
    ...
    /** @type {string} */
    let rol = e.target.id;
    if (rol.includes("diario")) rol = "diario";
    else if (rol.includes("semanal")) rol = "semanal";
    else if (rol.includes("quincenal")) rol = "quincenal";
    else if (rol.includes("mensual")) rol = "mensual";
...
const moduloInventario = (contenedor) => {
  ...
  mostrarDetails(contenedor.querySelector("details"));
  registrarProducto(
    contenedor.querySelector("#registrarProducto"),
...
const moduloNuevaCompra = (contenedor) => {
  ...
  registrarProveedor(
    contenedor.querySelector("#registrarProveedor"),
  ...
  $('a[role="navegacion"]').each((_i, enlace) => {
    enlace.addEventListener("click", (e) => {
      ...
      $.get(enlace.getAttribute("href"), (res) => {
        ...
        if (
          enlace.href.includes("usuarios.php") ||
          ...
          if (
            enlace.parentElement.previousElementSibling &&
            enlace.parentElement.parentElement
          ) {
            enlace.parentElement.previousElementSibling.classList.add(
              "w3-blue",
            );
        ...
        if ($("#moduloProveedores")[0])
          moduloProveedores($("#moduloProveedores")[0]);
        ...
        if ($("#moduloInventario")[0])
          moduloInventario($("#moduloInventario")[0]);
        if ($("#moduloVentas")[0]) moduloVentas($("#moduloVentas")[0]);
        if ($("#moduloCompras")[0]) moduloCompras($("#moduloCompras")[0]);
        if ($("#moduloNuevaCompra")[0])
          moduloNuevaCompra($("#moduloNuevaCompra")[0]);
        if ($("#moduloNuevaVenta")[0])
          moduloNuevaVenta($("#moduloNuevaVenta")[0]);
      });
    });
  });
